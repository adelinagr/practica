...
export function NewsletterSection() {
  ...
  return (
    <section className="relative overflow-hidden rounded-3xl bg-indigo-600 px-6 py-16 sm:px-12 sm:py-24 md:px-16 lg:flex lg:items-center lg:justify-between">
      ...
      <div className="absolute inset-y-0 right-1/2 -z-10 mr-16 w-[200%] origin-bottom-left skew-x-[-30deg] bg-white ring-1 ring-indigo-50 shadow-xl shadow-indigo-600/10 sm:mr-28 lg:mr-0 xl:mr-16 xl:origin-center" />
      ...
      <div className="mt-10 lg:mt-0 lg:w-1/2 lg:max-w-md lg:shrink-0">
        <form onSubmit={handleSubmit} className="relative mt-6 sm:flex sm:max-w-md">
          ...
          <input
            ...
            className="w-full min-w-0 appearance-none rounded-xl border-0 bg-white/10 px-4 py-3.5 text-base text-white shadow-sm ring-1 ring-inset ring-white/20 placeholder:text-indigo-200 focus:ring-2 focus:ring-inset focus:ring-white sm:w-auto sm:flex-auto"
            placeholder="Introduceți adresa de email"
          />
          <Button
            type="submit"
            disabled={status === "loading" || status === "success"}
            className="mt-4 flex w-full items-center justify-center rounded-xl bg-white px-6 py-3.5 text-sm font-semibold text-indigo-600 shadow-sm transition-all hover:bg-indigo-50 sm:mt-0 sm:ml-4 sm:w-auto"
          >
            {status === "loading" ? (
